...
## 与 CL LP Rebalancer 的区别

| 维度 | CL LP Rebalancer | Cross Funding Arb |
|------|------------------|-------------------|
| 收益来源 | LP 手续费（做市） | 资金费率差（套利） |
| 交易所 | 链上 DEX（Uniswap V3） | CEX/DEX 永续（HL + Binance） |
| 持仓形式 | NFT LP Position | 双腿永续合约（一多一空） |
| 风险特征 | 无常损失 (IL) | 费率反转 + 单腿脱钩 |
| 核心参数 | 范围宽度、波动率倍数 | 最低 APR、稳定性快照数 |
| 执行层 | onchainos CLI → 链上 | SDK 直连 HL + Binance API |
| 杠杆 | 无（LP 无杠杆） | 可配置（默认 3x） |
| 币种选择 | 固定交易对 | 自动扫描最优币种 |
| 调仓逻辑 | 波动率驱动范围调整 | Spread 驱动切仓/平仓 |
...
## Tool Wrapper: Exchange Clients

### Hyperliquid Client (HLClient)

认证方式：EIP-712 私钥签名，无需 API Key。支持 Agent Wallet（子密钥，可交易不可转账）。

```python
# 行情
hl.get_mid_price("ETH")           # 中间价
hl.get_funding_rate("ETH")        # 8h 资金费率
hl.get_all_funding_rates()        # 所有币种费率

# 账户
hl.get_usdc_balance()             # USDC 余额（perp + spot）
hl.get_position("ETH")            # 持仓信息
hl.get_all_positions()            # 所有持仓

# 交易
hl.set_leverage("ETH", 3, cross=True)
hl.market_order("ETH", is_buy=True, size=1.0, slippage=0.001)
hl.limit_order("ETH", is_buy=True, size=1.0, price=2000)
...
认证方式：HMAC-SHA256 签名。自动时间同步（往返中点校准）。

```python
# 行情
bn.get_mid_price("ETH")           # 标记价格
bn.get_funding_rate("ETH")        # 最新费率
bn.get_all_funding_rates()        # 所有 USDT-M 费率

# 账户
bn.get_usdt_balance()             # USDT 余额
bn.get_position("ETH")            # 持仓信息

# 交易
bn.set_leverage("ETH", 3)
bn.market_order("ETH", is_buy=True, size=1.0, slippage=0.001)
bn.close_position("ETH")
```
