...
description: "Agent Team 工厂：协调 5 个 AI Agent（Strategy/Backtest/Infra/Publish/Iteration）完成 OKX OnchainOS 链上交易策略的全生命周期——开发、回测、部署、发布、迭代。触发词：策略开发、agent team、回测、迭代策略、发布 skill、部署策略。"
license: Apache-2.0
metadata:
...
- 开发新交易策略 → 读 `roles/lead.md`，spawn strategy + backtest
- 回测 → spawn backtest，读 `roles/backtest.md`
- 部署到 VPS → spawn infra，读 `roles/infra.md`
- 发布为独立 Skill → spawn publish，读 `roles/publish.md`
- 迭代/复盘 → spawn iteration，读 `roles/iteration.md`
- 全流程 → spawn 全部 teammate，Lead 协调
...
2. Strategy agent 输出到 `Strategy/Script/v{version}/`
3. Lead 验证产出完整性
...
**Input**: Step 1 输出的 `Strategy/Script/v{version}/`

**Actions**:
1. Spawn backtest teammate
2. 拉取历史行情数据
3. 运行回测，输出到 `Strategy/Backtest/v{version}/`
...
3. 生成独立 Skill 包到 `Skills/{strategy-name}/`
...
3. 输出到 `Strategy/Iteration/v{version}-review-{date}.md`
...
Track in `state.json`。Log every transition: `[STATE] {name} v{ver}: {OLD} → {NEW} | {reason}`
