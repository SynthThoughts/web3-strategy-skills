...
- SSH 握手成功
- 磁盘 > 1GB，内存 > 512MB
- 备份当前版本：`cp -r /opt/strategy/current /opt/strategy/backup-v{old}`

### 3. Deploy
```bash
scp -i /tmp/deploy_key -r Strategy/Script/v{ver}/ user@host:/opt/strategy/staging/
ssh -i /tmp/deploy_key user@host 'cd /opt/strategy && pm2 stop strategy-bot; \
...
```bash
ssh user@host 'cd /opt/strategy && pm2 stop strategy-bot; rm -rf current; mv current.old current; \
